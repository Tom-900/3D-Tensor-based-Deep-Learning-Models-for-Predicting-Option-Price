...
import os
from core.models_CNN_RNN import model
...
def get_models(args):
    if args.test_checkpoint is None:
        net = model(resolution_ratio=4, nonlinearity="tanh", in_dim=5, in_channels=3, out_dim=1, seq_len=10)
...
if __name__ == '__main__':
    ...
    train_input = torch.load(r"\train_input.pt").float().cuda()
    ...
    train_label = torch.load(r"\train_label.pt").float().cuda()
    ...
    test_input = torch.load(r"\test_input.pt").float().cuda()
    ...
    test_label = torch.load(r"test_label.pt").float().cuda()
    ...
    # use the following if you need only 1 channel:
    # train_input = train_input.reshape(train_input.shape[0], 1, train_input.shape[0], train_input.shape[0])
    # test_input = test_input.reshape(test_input.shape[0], 1, test_input.shape[0], test_input.shape[0])
    ...
    # parser.set_defaults(test_checkpoint="May15_161233")
    # args = parser.parse_args()
    # test(test_loader, args)
